...
public class MyNeuralNetwork : MonoBehaviour
{
    ...
    {
        debugText.text += "try open NNFile\n";

        // nNData = File.OpenText("NNData/data.txt");
        TextAsset txtAsset = Resources.Load<TextAsset>("NNData/data");
        nNData = new StringReader(txtAsset.text);

        int dataCounter = 0;

        if (nNData == null)
        {
            debugText.text += "NNFile Not Found\n";
            Debug.Log("NNFileNotFound");
            return;
        }
        debugText.text += "loaded NNFile\n";
    ...
    private void SaveData()
    {
        ...
        string filePath = Path.Combine(Application.persistentDataPath, "NNData/data.txt.txt");
        dataWrite = new StreamWriter(filePath, false);
